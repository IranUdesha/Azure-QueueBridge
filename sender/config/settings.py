...
class Settings:
    ...
    connection_string: str              # Full Azure Storage connection string (built from account name/key if not provided directly)
    queue_name: str                     # Name of the Azure Storage Queue to send/receive messages
    ...
    api_key_header_name: str            # HTTP header name to read the API key from (default: X-API-Key)
...
def load_settings(dotenv_path: Optional[str] = None) -> Settings:
    ...
    # Default .env location: one directory up from config/ (i.e. the sender project root)
    if not dotenv_path:
